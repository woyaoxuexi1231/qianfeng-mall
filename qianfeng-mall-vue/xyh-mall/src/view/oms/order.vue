                ...
                <el-table-column label="操作" width="200" align="center">
                    <template slot-scope="scope">
                        ...
                        <el-button
                            size="mini"
                            @click="
                                handleDeliveryOrder(scope.$index, scope.row)
                            "
                        ...
                        <!-- <el-button
                            size="mini"
                            @click="
                                handleViewLogistics(scope.$index, scope.row)
                            "
                            v-show="
...
export default {
    ...
    methods: {
        ...
        handleBatchOperate() {
            ...
            if (this.operateType === 1) {
                ...
                for (let i = 0; i < this.multipleSelection.length; i++) {
                    ...
                    if (this.closeOrder.orderIds.length === 0) {
                    this.$message({
        ...
        getOrder(){
            this.$axios.get('orders/list?page='+this.listQuery.pageNum+'&limit='+this.listQuery.pageSize+'&orderId='+this.listQuery.orderId+'&receiver='+this.listQuery.receiverKeyword+'&orderType='+this.listQuery.status+'&sourceType='+this.listQuery.sourceType)
                .then(res => {
        ...
        closeOrderById(){
            this.$axios.get('orders/order/close?ids='+this.closeOrder.orderIds+'&note='+this.closeOrder.content)
                .then(res => {
                    this.closeOrder.orderIds = [];
        ...
        deleteOrderById(ids){
            this.$axios.get('orders/order/delete?ids='+ids)
                .then(res => {
                    this.$message({
                        message: "删除成功！",
                        type: "success",
